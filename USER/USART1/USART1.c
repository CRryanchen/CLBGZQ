...
static void USART1_TX_GpioInit(void)
{
    ...
    RCC_APB2PeriphClockCmd(USART1_TX_GPIO_CLK | RCC_APB2Periph_AFIO, ENABLE);// 开启GPIO时钟、AFIO时钟
...
static void USART1_RX_GpioInit(void)
{
    ...
    RCC_APB2PeriphClockCmd(USART1_RX_GPIO_CLK, ENABLE);// 开启GPIO时钟
...
static void USART1_ModeConfig(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1,ENABLE);                            // 使能串口时钟,串口1是APB2，串口2,3,4,5是APB1
    ...
    USART_Init(USART1, &USART_InitStructure);                                        // 初始化串口配置结构体

    USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);                                   // 需要使用串口接收中断的话，需要开启
...
{
    USART_SendData(USART1, l_ch);// USART1发送1字节数据
